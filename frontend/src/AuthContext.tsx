...
const LS_KEY = "devinci_auth_user";                     // local storage key for authed user (saved when refreshing)
...
export function AuthProvider({ children }: { children: React.ReactNode }) {  // wraps App
  ...
  useEffect(() => {                                     // runs after first render (because deps always [])
    const raw = localStorage.getItem(LS_KEY);           // get user from local storage
    if (!raw) return;                                   // if none, do nothing
    try {
      setUser(JSON.parse(raw));                         // try setting current user to saved user
    } catch {
      localStorage.removeItem(LS_KEY);                  // if error, remove invalid data
    }
  }, []);
  ...
  const value = useMemo<AuthState>(() => ({             // gets memoized auth context value
    ...
    login: (u) => {                                     // login function
      ...
      localStorage.setItem(LS_KEY, JSON.stringify(u));  // saves to local storage
    },
    logout: () => {                                     // logout function
      setUser(null);                                    // clears current user
      localStorage.removeItem(LS_KEY);                  // removes from local storage
    },
  }), [user]);
